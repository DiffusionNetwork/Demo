...
def load_data(graph_path, result_path):
    ...
    with open(graph_path, 'r') as f:
        ...
        np.savetxt('./netsci_network.txt',data,fmt='%d',delimiter=' ')
...
def make_incomplete(diffusion_result, missing_rate, nodes_num, incomplete_choice, missing_index_address):
    ...
    # incomplete_choice = 0 随机使观测数据缺失；  = 1 从文件中读取一个确定的缺失数据
...
def init_sample(incomplete_result, small_times, big_times, sample_choice):
    ...
    # sample_choice = 0 统计得到先验；  =1 初始化为0.5
    if sample_choice == 0:
        prior_prob = np.sum(tmp, axis=0) / (result_num - missing_num)  # 根据观察到的数据得到的，每个节点取1的先验概率
    else:
        prior_prob = 0.5 * np.ones(incomplete_result.shape[1])

    # 采样small_times * big_times次
    temp_results_list=[]
...
def cal_score(child, parents, record_states):
    ...
    nij[np.where(nij==0)]+=1  # 防止除0错误
    temp_count = count_states.copy()
    temp_count[np.where(temp_count==0)]+=1     # 防止log 0 错误
...
def cal_single_s(p_matrix, obs_result, network_stru, prior_prob, equation_flag = False):
    ...
    mis_nodes = np.where(obs_result == -1)[0]  # 感染状态missing的节点
    no_par_nodes = np.where(np.sum(network_stru, axis=0) == 0)[0]  # 没有父节点的节点
...
def myfunc_lambda(x, *args):
    ...
    for i in missing_index:
        ...
        inf_children = np.where(network_stru[i,:]*obs_result==1)[0]   # 感染状态为1的i的子节点集合
        ...
        uninf_children = np.where(network_stru[i,:]*temp==2)[0]  # 感染状态为0的i的子节点集合
        ...
        missing_children = np.where(network_stru[i,:]*obs_result==-1)[0]  # 感染状态缺失的i的子节点集合
    ...
    if missing_index.size>0:
        if equation_flag_lambda:
            ...
            while not stop_flag:
                ...
                pre_lambdas = cur_lambda.copy()      # 有时方程组无解
                ...
                cur_deltas = np.sum(abs(pre_lambdas-cur_lambda))     # 有时候方程组无解
...
def cal_mae_v2(groundtruth_network, p, infer_p):
    ...
    mae_v2 = np.sum(abs(infer_p-gt_p))/edges_num
...
def cal_sample_accuracy(results_list, incomplete_result, complete_result, small_times, big_times):
    incomplete_cnt=np.sum(incomplete_result==-1)
    accuracy_rate_sum=0
    for i in range(small_times*big_times):
        cur_result=results_list[i]
        error_cnt=np.sum(abs(cur_result-complete_result))
        accuracy_rate_sum+=(incomplete_cnt-error_cnt)/incomplete_cnt
    mean_accuracy=accuracy_rate_sum/(small_times*big_times)

    return mean_accuracy



def Pearson_prune(record_states):
    results_num, nodes_num = record_states.shape
    print("results_num = ", results_num)
    IMI = np.zeros((nodes_num, nodes_num))
